    ...
    <div id="hd_top">
        ...
        <div id="tnb">
            <ul>
                ...
                <?php } ?>
                <li class="tnb_li"><a href="<?php echo G5_ADMIN_URL ?>/admin_schedule.php?mode=app"
                        onclick="window.open(this.href, 'schedule_app', 'width=1200,height=900,scrollbars=yes'); return false;"
                        class="tnb_schedule" title="일정관리"><i class="fa fa-calendar-check-o" aria-hidden="true"></i>
                        일정관리</a></li>
...
<script>
    jQuery(function ($) {
        ...
        $(".tnb_mb_btn").click(function () { // FIX
            $(".tnb_mb_area").toggle();
        });

        $("#btn_gnb").click(function () { // FIX
        ...
        // Intercept ALL Schedule Links (Sidebar & Top)
        $(document).on('click', "a[href*='admin_schedule.php']", function (e) {
            e.preventDefault();
            var url = $(this).attr('href');
            // Ensure App Mode
            if (url.indexOf('mode=app') === -1) {
                url += (url.indexOf('?') === -1 ? '?' : '&') + 'mode=app';
            }
            window.open(url, 'schedule_app', 'width=1200,height=900,scrollbars=yes');
            return false;
        });
