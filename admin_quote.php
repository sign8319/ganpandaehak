...
// ============================================================================
// [FIX] 경로 수정 - theme/basic/includes에서 로드
$quote_func_path = G5_THEME_PATH . '/includes/quote_functions.php';
$quote_db_path = G5_THEME_PATH . '/includes/quote_db_schema.php';

// 테마 경로에서 먼저 시도, 없으면 상대 경로
if (file_exists($quote_func_path)) {
    include_once($quote_func_path);
} elseif (file_exists('./theme/basic/includes/quote_functions.php')) {
    include_once('./theme/basic/includes/quote_functions.php');
} else {
    error_log("[QUOTE ERROR] quote_functions.php not found! Paths tried: $quote_func_path, ./theme/basic/includes/quote_functions.php");
}

if (file_exists($quote_db_path)) {
    include_once($quote_db_path);
} elseif (file_exists('./theme/basic/includes/quote_db_schema.php')) {
    include_once('./theme/basic/includes/quote_db_schema.php');
} else {
    error_log("[QUOTE ERROR] quote_db_schema.php not found!");
}
...
if ($w == 'u' || $w == 'c') {
    ...
    // [NEW] Sync to Work Table - with fallback and logging

    // 변수 체크 및 기본값 강제 할당
    $sync_subject = !empty($qa_subject) ? $qa_subject : (isset($_POST['qa_subject']) && trim($_POST['qa_subject']) ? sql_real_escape_string(trim($_POST['qa_subject'])) : '제목없음');
    $sync_status = !empty($qa_status) ? $qa_status : (isset($_POST['qa_status']) && trim($_POST['qa_status']) ? sql_real_escape_string(trim($_POST['qa_status'])) : '작성중');
    $sync_customer_id = !empty($qa_customer_id) ? (int)$qa_customer_id : 0;

    error_log("[QUOTE SYNC] START - qa_id: $qa_id, subject: $sync_subject, status: $sync_status, customer_id: $sync_customer_id");
    ...
    if ($work_check && $work_check['work_id']) {
        // 기존 work 레코드 업데이트
        $fallback_sql = " UPDATE g5_work SET
                            work_subject = '$sync_subject',
                            work_status = '$sync_status',
                            customer_id = '$sync_customer_id',
                            updated_at = '$sync_now'
                          WHERE work_id = '{$work_check['work_id']}' ";
        echo "SQL: " . $fallback_sql; exit;
        $fallback_result = sql_query($fallback_sql, false);
        if (!$fallback_result) {
            error_log("[QUOTE SYNC] FALLBACK UPDATE SQL ERROR: " . sql_error());
        }
        error_log("[QUOTE SYNC] FALLBACK UPDATE - work_id: {$work_check['work_id']}, result: " . ($fallback_result ? 'OK' : 'FAIL'));
    } else {
        // 새 work 레코드 생성
        $fallback_sql = " INSERT INTO g5_work SET
                            work_subject = '$sync_subject',
                            work_status = '$sync_status',
                            qa_id = '$qa_id',
                            customer_id = '$sync_customer_id',
        ...
        $fallback_result = sql_query($fallback_sql, false);
        if (!$fallback_result) {
            error_log("[QUOTE SYNC] FALLBACK INSERT SQL ERROR: " . sql_error());
        }
        $new_work_id = sql_insert_id();
        error_log("[QUOTE SYNC] FALLBACK INSERT - new work_id: $new_work_id, result: " . ($fallback_result ? 'OK' : 'FAIL'));
    }

    // 함수가 정의되어 있으면 추가로 실행 (이중 안전장치)
    if (function_exists('save_work_from_quote')) {
        error_log("[QUOTE SYNC] Calling save_work_from_quote($qa_id)");
        save_work_from_quote($qa_id);
        error_log("[QUOTE SYNC] save_work_from_quote completed");
    } else {
        error_log("[QUOTE SYNC] WARNING - save_work_from_quote function NOT FOUND! Using fallback only.");
    }

    error_log("[QUOTE SYNC] END - qa_id: $qa_id");
