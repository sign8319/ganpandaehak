...
if ($w == "") {
    ...
    $tmp_mb_id = isset($_POST['mb_id']) ? $_POST['mb_id'] : (isset($_GET['mb_id']) ? $_GET['mb_id'] : '');
    if ($member['mb_social_id'] && !$tmp_mb_id)
        $tmp_mb_id = $member['mb_id'];

    if ($member['mb_id'] != $tmp_mb_id)
    ...
    */
    ...
    if ($_POST['mb_id'] && !(isset($_POST['mb_password']) && $_POST['mb_password'])) {
        if (!$is_social_login_modify && !$member['mb_social_id']) {
            alert('비밀번호를 입력해 주세요.');
        }
    }
