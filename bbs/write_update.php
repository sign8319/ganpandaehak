...
if ($is_use_captcha && !chk_captcha() && $config[cf_captcha] !== 'recaptcha') {
...
// consult 게시판인 경우 리다이렉트하지 않고 성공 메시지만 표시
if ($bo_table == 'consult') {
    // FIX: 간판상담문의(g5_write_consult) 저장 시 g5_quote 테이블에도 데이터 INSERT
    $qa_code_prefix = 'Q-' . date('Ymd') . '-'; // FIX
    $qa_row = sql_fetch(" select count(*) as cnt from g5_quote where qa_code like '{$qa_code_prefix}%' "); // FIX
    $qa_seq = $qa_row['cnt'] + 1; // FIX
    $qa_code = $qa_code_prefix . sprintf('%03d', $qa_seq); // FIX

    $qa_client_name = isset($wr_name) ? $wr_name : ''; // FIX: 이미 addslashes 처리됨
    $qa_client_email = isset($wr_email) ? $wr_email : ''; // FIX: 이미 addslashes 처리됨
    $qa_subject = isset($wr_subject) ? $wr_subject : ''; // FIX: 이미 addslashes 처리됨
    // wr_content는 이미 addslashes 처리되어 있음. 따옴표 중복 방지를 위해 stripslashes 후 다시 escaping 하거나 그대로 사용 고려.
    // write_update.php 상단에서 $wr_content는 normalization과 trim 등 처리됨. addslashes는 clean_xss_tags 처리 시에는 안되었을 수도 있으나, insert into $write_table 할때는 raw string이 들어가는 구조인지 확인 필요. 
    // line 301 sql을 보면 '$wr_content' 로 들어감. 즉 escaping이 안된 상태일 수 있음 (prepare statement가 아니므로).
    // 하지만 G5 core 로직상 magic_quotes가 없으면 addslashes를 해야 함.
    // 기존 코드 line 274: wr_content = '$wr_content' -> 여기서 $wr_content는 escaping이 안되어 있다면 SQL Injection 발생.
    // G5는 기본적으로 common.php에서 POST 데이터를 escape 처리하거나, write_update.php에서 처리함.
    // 확인결과: $wr_name 등은 위에서 addslashes 처리됨. $wr_content는?
    // line 43: $wr_content = substr(trim($_POST['wr_content']),0,65536);
    // line 237: $wr_name = addslashes(...);
    // wr_content는 별도 escaping이 안보임. (line 712 mail sending 부분에서 stripslashes 사용함).
    // line 274 insert 문에서 '$wr_content'를 쓰고 있음. 이는 $wr_content가 safe하다는 가정. (혹은 G5 설정에 의존).
    // 안전을 위해 sql_real_escape_string 사용.
    $qa_memo_user = sql_real_escape_string(isset($_POST['wr_content']) ? $_POST['wr_content'] : ''); // FIX

    // wr_phone은 write_update.php 표준 변수가 아니므로 POST에서 직접 수신
    $qa_client_hp = isset($_POST['wr_phone']) ? sql_real_escape_string(trim($_POST['wr_phone'])) : ''; // FIX

    $qa_sql = " insert into g5_quote
                set qa_code = '{$qa_code}',
                    qa_status = '작성중',
                    qa_subject = '{$qa_subject}',
                    qa_client_name = '{$qa_client_name}',
                    qa_client_hp = '{$qa_client_hp}',
                    qa_client_contact = '{$qa_client_hp}',
                    qa_client_email = '{$qa_client_email}',
                    qa_memo_user = '{$qa_memo_user}',
                    qa_datetime = '" . G5_TIME_YMDHIS . "' "; // FIX
    sql_query($qa_sql); // FIX

    if ($file_upload_msg)
        alert($file_upload_msg, G5_URL);
    else
        alert('상담신청이 완료되었습니다.', G5_URL . '/', false);
} else {
