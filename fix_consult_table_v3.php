...
echo "<h1>Fix Consult Table & Data v3</h1>\n";

$target_table = $g5['write_prefix'] . 'consult';

// 0. Adjust SQL Mode for Session
sql_query("SET SESSION sql_mode = ''");

// 1. Create Table (Direct Schema)
$row = sql_fetch(" SHOW TABLES LIKE '{$target_table}' ");
if ($row) {
    echo "<div>Table '{$target_table}' already exists.</div>\n";
} else {
    echo "<div>Creating table '{$target_table}'...</div>\n";
    ...
    $res = sql_query($sql);
    if (!$res) {
        echo "<div>FAILED to create table: " . sql_error_info() . "</div>\n";
    } else {
...
while ($row = sql_fetch_array($result)) {
    ...
    // PHP < 8.0 support for array access on bool/null check
    $dup_cnt = isset($check_row['cnt']) ? $check_row['cnt'] : 0;
    ...
    if ($res_ins) {
        ...
        echo "<div>Failed to insert: {$wr_subject}</div>\n";
    }
}
