...
// 사용자 정의: SQL을 통해 추가한 mb_social_id 컬럼 사용 (VARCHAR 255)

// 먼저 mb_social_id로 회원이 있는지 조회
$mb = sql_fetch(" select * from {$g5['member_table']} where mb_social_id = '{$naver_id}' ");

if (!$mb['mb_id']) {
    // 신규 회원이면 자동 가입 처리

    // mb_id 생성: 그누보드 mb_id(20자) 제한 대응.
    // 조회는 mb_social_id로 하므로 mb_id는 식별용으로만 유니크하면 됩니다.
    $mb_id = 'n' . substr(md5($naver_id), 0, 18);

    // mb_id 중복 체크
    $check = sql_fetch(" select mb_id from {$g5['member_table']} where mb_id = '{$mb_id}' ");
    if ($check['mb_id'])
        $mb_id = 'n' . substr(md5($naver_id . time()), 0, 18);

    $mb_name = isset($user_data['response']['name']) ? $user_data['response']['name'] : '네이버사용자';
    $mb_nick = isset($user_data['response']['nickname']) ? $user_data['response']['nickname'] : $mb_name;
    $mb_email = isset($user_data['response']['email']) ? $user_data['response']['email'] : '';
    ...
    $sql = " insert into {$g5['member_table']}
                set mb_id = '{$mb_id}',
                     ...
                     mb_9 = '',
                     mb_10 = '' ";
    // 이제 mb_1은 사용하지 않고 mb_social_id에 직접 전체 ID를 저장합니다.
