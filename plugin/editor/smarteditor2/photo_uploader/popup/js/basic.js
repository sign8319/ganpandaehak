...
jQuery(function ($) {
    ...
    var gnu = {
        ...
        pending_data: [], // [NEW] Store pending uploads
        init: function () {
            // [NEW] Inject CSS to fix layout for SEO inputs and Delete button
            $('<style>')
                .prop('type', 'text/css')
                .html(`
                    .sortable > li { width: 120px !important; height: auto !important; min-height: 150px; overflow: visible !important; }
                    .sortable > li > div { height: auto !important; overflow: visible !important; padding-bottom: 5px; }
                    .sortable > li > div .delete_img { display: block !important; width: 20px; height: 20px; top: 2px; right: 2px; z-index: 10; }
                    .sortable > li > div .delete_img img { width: 100%; height: 100%; }
                    .rename-input { display: block !important; width: 90% !important; margin: 5px auto !important; z-index: 20; position: relative; }
                `)
                .appendTo('head');
            ...
            $(this.dreg_area_list).sortable({
                ...
                'cancel': 'input,textarea,button,select,option' // [NEW] Allow interaction with inputs
            });
            // $(this.dreg_area_list).disableSelection(); // [REMOVED] This blocks input focus
        ...
        // ... (skipping unchanged properties if possible, but replace_file_content needs contiguous)
        // Actually, let's just add pending_data at the top of gnu object
        ...
        _add: function (e, data, preload) {
            ...
            // [NEW] Store data for manual submit if not preloading
            if (!preload) {
                othis.pending_data.push(data);
            }
            ...
            $.each(data.files, function (index, file) {
                ...
                // [NEW] SEO Filename Input
                var inputHtml = '';
                if (!preload) {
                    var nameWithoutExt = file.name.substring(0, file.name.lastIndexOf('.')) || file.name;
                    inputHtml = '<br><input type="text" class="rename-input" style="width:90%; margin-top:5px; font-size:11px;" value="' + nameWithoutExt + '" placeholder="SEO 이름">';
                }
                ...
                if (!index) {
                    ...
                    // Append Input
                    if (inputHtml) {
                        node.append(inputHtml);
                        // [NEW] Explicitly stop propagation to allow focus
                        node.find('.rename-input').on('click mousedown', function (e) {
                            e.stopPropagation();
                            $(this).focus();
                        });
                    }

                    // [NEW] Local Preview using FileReader (Simplified)
                    if (!preload && othis.file_api_support && file.type.match('image.*')) {
                        var reader = new FileReader();
                        reader.onload = function (e) {
                            // Directly set src and style to ensure visibility
                            node.find('.pre_thumb').attr('src', e.target.result).css({
                                'width': 'auto',
                                'height': 'auto',
                                'max-width': '100px',
                                'max-height': '100px'
                ...
                node.find(".delete_img").on("click", function (e) {
                    // [NEW] Remove from pending_data on delete
                    var idx = othis.pending_data.indexOf(data);
                    if (idx > -1) othis.pending_data.splice(idx, 1);
                    othis._delete(e);
                });
            });
    ...
    $("#img_upload_submit").bind("click", function (e) {
        ...
        // [NEW] Process Pending Uploads first
        if (gnu.pending_data.length > 0) {
            var $btn = $(this);
            $btn.find('span').text('업로드 중...');

            var promises = [];

            $.each(gnu.pending_data, function (i, data) {
                var deferred = $.Deferred();

                // Rename files
                $.each(data.files, function (fv, file) {
                    var input = data.context.find('.rename-input');
                    if (input.length) {
                        var newBaseName = input.val();
                        var ext = file.name.split('.').pop();
                        // Construct new name
                        var newName = newBaseName + '.' + ext;

                        // Try to manipulate file object
                        try {
                            // Modern browsers
                            var newFile = new File([file], newName, { type: file.type });
                            data.files[fv] = newFile;
                        } catch (e) {
                            // Fallback?
                        }
                    }
                ...
                data.submit().success(function () {
                    ...
                    deferred.resolve(); // Resolving anyway to continue
                });
            ...
            $.when.apply($, promises).then(function () {
                gnu.pending_data = []; // Clear queue
                $btn.find('span').text('등록');
                // Call submit again (or proceed)
                proceedSubmit();
            });
