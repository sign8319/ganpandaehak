...
if ($w == 'status' && $es_id) {
    $st = isset($_REQUEST['st']) ? clean_sql($_REQUEST['st']) : '대기';
    $res = sql_query(" update g5_estimate set es_status = '{$st}' where es_id = '{$es_id}' ");
    if (!$res) {
...
// Save Estimate
if ($w == 'submit') {
    $es_company = isset($_POST['es_company']) ? clean_sql($_POST['es_company']) : '';
    $es_email = isset($_POST['es_email']) ? clean_sql($_POST['es_email']) : '';
    $es_code = isset($_POST['es_code']) ? clean_sql($_POST['es_code']) : ''; // For update
                ...
                <table style="width:100%; border-collapse:collapse; margin-bottom:20px;">
                    ...
                    <tfoot>
                        ...
                        <tr>
                            ...
                            <td
                                style="padding:10px; border:1px solid #ddd; text-align:right; font-weight:bold; background:#fff7ed;">
                                <?php echo number_format($es_vat); ?>원
                            </td>
                        </tr>
                        <tr>
                            <td colspan="4"
                            ...
                            <td
                                style="padding:10px; border:1px solid #ddd; text-align:right; font-weight:bold; color:#d97706; font-size:1.2em;">
                                <?php echo number_format($es_total); ?>원
                            </td>
                        </tr>
                    </tfoot>
                </table>
        ...
        <?php if ($w == '' || $w == 'status'): // ------------------------ LIST VIEW ------------------------ ?>
            ...
            <div class="container mx-auto px-4 py-8 min-h-screen">
                ...
                <form class="mb-6 flex gap-2" method="get">
                    <input type="text" name="q" value="<?php echo e($q) ?>" placeholder="업체명 검색"
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            while ($row = sql_fetch_array($result)) {
                                ...
                                <tr class="hover:bg-orange-50 transition duration-150">
                                    <td class="p-4 font-mono text-sm"><?php echo e($row['es_code']); ?></td>
                                    <td class="p-4 font-bold text-gray-800"><?php echo e($row['es_company']); ?></td>
                                    <td class="p-4 text-right font-bold text-orange-600">
                                        <?php echo number_format($row['es_total']); ?>원
                                    </td>
                                    <td class="p-4 text-center text-gray-500 text-sm">
                                        <?php echo substr($row['es_datetime'], 0, 10); ?>
                                    </td>
                                    <td class="p-4 text-center">
                                        <span class="px-3 py-1 rounded-full text-xs font-bold <?php echo e($status_cls); ?>">
                                            <?php echo e($row['es_status']); ?>
                                        </span>
                                    </td>
                    ...
                    <form name="festimate" action="?w=submit" method="post" onsubmit="return fsubmit(this);" class="p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            <div>
                                ...
                                <input type="text" name="es_company" value="<?php echo e($estimate['es_company']); ?>"
                                    required
                            ...
                            <div>
                                ...
                                <input type="email" name="es_email" value="<?php echo e($estimate['es_email']); ?>" required
                                    class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:outline-none">
                            </div>
                        </div>
                                        ...
                                        <?php foreach ($items as $k => $item) { ?>
                                            <tr class="item-row">
                                                ...
                                                <td class="p-2"><input type="text" name="ei_spec[]"
                                                        value="<?php echo e($item['ei_spec']); ?>"
                                                        class="w-full p-2 border rounded focus:ring-1 focus:ring-orange-500 outline-none"
                                                        placeholder="규격"></td>
                                                <td class="p-2"><input type="number" name="ei_qty[]"
                                                        value="<?php echo e($item['ei_qty']); ?>"
                                                ...
                                                <td class="p-2"><input type="number" name="ei_price[]"
                                                        value="<?php echo e($item['ei_price']); ?>"
                                                ...
                                                <td class="p-2 text-right font-semibold text-gray-700 in-total">
                                                    <?php echo number_format($item['ei_amount']); ?>
                                                </td>
                                                <td class="p-2 text-center"><button type="button" onclick="del_row(this)"
                                                        class="text-red-400 hover:text-red-600 text-xl font-bold">&times;</button>
                                                </td>
                                            </tr>
                                        <?php } ?>
                                    </tbody>
                        ...
                        <div class="flex items-center mb-8 bg-blue-50 p-4 rounded-lg border border-blue-100">
                            ...
                            <label for="send_mail" class="ml-3 text-gray-800 font-medium cursor-pointer select-none">저장 후
                                이메일로 견적서
                                발송하기</label>
                        </div>
