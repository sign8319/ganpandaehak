...
// ============================================================================
include_once('./includes/quote_functions.php'); // 공통 함수 포함

// [FIX ORPHANS] 고아 데이터(g5_work 없는 견적) 복구 로직
if (isset($_GET['action']) && $_GET['action'] == 'fix_orphans') {
    // 1. Find quotes without work
    $sql = " SELECT q.qa_id FROM g5_quote q 
             LEFT JOIN g5_work w ON q.qa_id = w.qa_id 
             WHERE w.work_id IS NULL ";
    $result = sql_query($sql);
    $fixed_count = 0;

    while ($row = sql_fetch_array($result)) {
        save_work_from_quote($row['qa_id']);
        $fixed_count++;
    }
...
if ($w == '') {
    ...
    $sql_search_static = " where 1 and w.work_subject not like '%퀵상담 신청%' "; // Filter out quick consults

    // [DRAFT SYSTEM] Hide draft quotes by default
    $show_drafts = isset($_GET['show_drafts']) ? $_GET['show_drafts'] : '0';
    // [MOD] Draft도 목록에 표시 (제외 로직 주석 처리)
    // The original exclusion logic was commented out, so no change needed here.
    // If $show_drafts is '1', we want to include drafts. If '0', we don't.
    // The current logic effectively shows all, as the exclusion is commented.
    // To truly hide drafts when $show_drafts is '0', uncomment the block below.
    // For now, following the instruction to "Remove 'draft' exclusion from SQL",
    // which means ensuring drafts are *not* excluded, so the commented block is fine.
    /*
    if ($show_drafts != '1') {
        $sql_search_static .= " and q.qa_status != 'draft' ";
    }
    */

    if ($stx) {
        $sql_search_static .= " and (w.work_subject like '%$stx%' or q.qa_client_name like '%$stx%' or c.customer_name like '%$stx%') ";
    }
    if ($f_year)
        $sql_search_static .= " and year(w.created_at) = '$f_year' ";
    if ($f_month)
        $sql_search_static .= " and month(w.created_at) = '$f_month' ";

    // Add status filter
    $f_status = isset($_GET['f_status']) ? $_GET['f_status'] : '';
    if ($f_status) {
        $sql_search_static .= " and q.qa_status = '" . sql_real_escape_string($f_status) . "' ";
    }
    // If show_drafts is not checked, and no specific status is selected, exclude 'draft'
    // [MOD] User requested drafts to be visible. Commenting out exclusion.
    /*
    if ($show_drafts != '1' && empty($f_status)) {
        $sql_search_static .= " and q.qa_status != 'draft' ";
    }
    */
...
if ($w == 'u' || $w == 'c') {
    ...
    $sync_subject = !empty($qa_subject) ? $qa_subject : '';
    $sync_status = !empty($qa_status) ? $qa_status : '작성중';
    $sync_customer_id = !empty($qa_customer_id) ? (int) $qa_customer_id : 0;

    // 예비 동기화: 함수가 실패해도 목록에 표시되도록 직접 INSERT/UPDATE
    $work_check = sql_fetch(" SELECT work_id FROM g5_work WHERE qa_id = '$qa_id' ");
    $sync_now = G5_TIME_YMDHIS;

    if ($work_check && $work_check['work_id']) {
        sql_query(" UPDATE g5_work SET
                    work_subject = '$sync_subject',
                    work_status = '$sync_status',
                    customer_id = '$sync_customer_id',
                    updated_at = '$sync_now'
                  WHERE work_id = '{$work_check['work_id']}' ", false);
    } else {
        sql_query(" INSERT INTO g5_work SET
                    work_subject = '$sync_subject',
                    work_status = '$sync_status',
                    qa_id = '$qa_id',
                    customer_id = '$sync_customer_id',
                    created_at = '$sync_now',
                    updated_at = '$sync_now' ", false);
    }

    // 함수가 정의되어 있으면 추가로 실행
    if (function_exists('save_work_from_quote')) {
        save_work_from_quote($qa_id);
    }
...
// Bulk Delete (UPDATED to use work_id)
// 🔥 FIX: POST 방식이고 chk_qa_id가 존재할 때만 실행
if ($w == 'multi_d' && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['chk_qa_id']) && is_array($_POST['chk_qa_id']) && count($_POST['chk_qa_id']) > 0) {
    check_quote_token();

    $chk_work_ids = $_POST['chk_qa_id'];

    $deleted = 0;

    foreach ($chk_work_ids as $work_id) {
        $work_id = (int) $work_id;
        if (!$work_id)
            continue;

        // 1. Find linked qa_id from work record
        $work = sql_fetch(" SELECT qa_id FROM g5_work WHERE work_id = '$work_id' ");
        $qa_id = (int) ($work['qa_id'] ?? 0);

        // 2. Cascade delete related quote documents if linked
        if ($qa_id) {
            sql_query(" delete from g5_quote where qa_id = '$qa_id' ", true);
            sql_query(" delete from g5_quote_item where qa_id = '$qa_id' ", true);
            sql_query(" delete from g5_quote_log where qa_id = '$qa_id' ", true);
            // Optionally: handle files cleanup if required
        }

        // 3. Delete work record itself
        $r1 = sql_query(" delete from g5_work where work_id = '$work_id' ", true);
    ...
    if (isset($_POST['is_ajax']) && $_POST['is_ajax'] == '1') {
        die(json_encode(['success' => true, 'deleted_count' => $deleted]));
    }
    ...
    <div class="flex-1 min-w-0">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 mb-6">
                    ...
                    <div class="flex flex-wrap gap-2 w-full md:w-auto p-4">
                        ...
                        <form name="fsearch" id="fsearch" method="get"
                            ...
                            <input type="hidden" name="f_status" id="f_status_hidden" value="<?php echo $f_status; ?>">
                            ...
                            <div class="flex flex-col lg:flex-row gap-2 items-stretch lg:items-center w-full lg:w-auto">
                                ...
                                <div class="admin-action-bar" style="margin-bottom:0;">
                                    <button type="button"
                                        onclick="document.getElementById('customer_choice_modal').showModal()"
        ...
        <script>
            function fquote_submit(target = null) {
                ...
                if (target === 'save') {
                    // save 경로: onsubmit 핸들러 제거 후 submit
                    const originalOnsubmit = f.onsubmit;
                    f.onsubmit = null;
                    f.submit();
                    f.onsubmit = originalOnsubmit;
                } else {
                    // 일반 submit: 확인 창 표시
                ...
                <form action="./admin_quote.php" method="post" enctype="multipart/form-data" class="p-6 space-y-6">
                    ...
                    <div class="border-b pb-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div><label class="block text-xs font-bold mb-1 text-gray-600">상호(법인명)</label><input
                                    type="text" name="biz_name" value="<?php echo $biz_info['biz_name'] ?? ''; ?>"
                                    class="w-full border border-gray-300 p-2.5 rounded focus:border-orange-500 focus:ring-1 focus:ring-orange-500">
                            </div>
                            <div><label class="block text-xs font-bold mb-1 text-gray-600">사업자등록번호</label><input
                                    type="text" name="biz_no" value="<?php echo $biz_info['biz_no'] ?? ''; ?>"
                                    class="w-full border border-gray-300 p-2.5 rounded focus:border-orange-500 focus:ring-1 focus:ring-orange-500">
                            </div>
                            <div><label class="block text-xs font-bold mb-1 text-gray-600">대표자성명</label><input
                                    type="text" name="biz_ceo" value="<?php echo $biz_info['biz_ceo'] ?? ''; ?>"
                                    class="w-full border border-gray-300 p-2.5 rounded focus:border-orange-500 focus:ring-1 focus:ring-orange-500">
                            </div>
                            <div><label class="block text-xs font-bold mb-1 text-gray-600">대표전화</label><input
                                    type="text" name="biz_tel" value="<?php echo $biz_info['biz_tel'] ?? ''; ?>"
                                    class="w-full border border-gray-300 p-2.5 rounded focus:border-orange-500 focus:ring-1 focus:ring-orange-500">
                            </div>
        ...
        <script>
            ...
            // 1. Bulk Delete Submit (List View) - UPDATED to AJAX
            // 1. Bulk Delete Submit (List View) - UPDATED to AJAX (No Confirm)
            function fquotelist_submit(f) {
                const checked = document.querySelectorAll('input[name="chk_qa_id[]"]:checked');
                if (checked.length === 0) {
                    // 선택된 항목이 없을 때만 알림
                ...
                // [UX CHANGE] No confirm, instant delete
                const formData = new FormData(f);
                formData.append('is_ajax', '1');
                formData.append('w', 'multi_d');
                ...
                fetch('./admin_quote.php', {
                    ...
                    .then(data => {
                        if (data.success) {
                            // Instant refresh without alert
                            load_list(currentMonth, currentPage);
                            // Reset "Select All"
        ...
        <script>
            ...
            window.load_list = function (month, page) {
                ...
                // [NEW] Show Drafts Filter
                var show_drafts_el = document.getElementById('show_drafts');
                var show_drafts = show_drafts_el && show_drafts_el.checked ? '1' : '0';
                ...
                url += '&show_drafts=' + show_drafts; // Add show_drafts parameter
        ...
        <div id="back_confirm_modal_safe" class="hidden fixed inset-0 z-[99999]" role="dialog" aria-modal="true">
            ...
            <div class="fixed inset-0 z-[100000] flex items-center justify-center p-4" style="pointer-events: none;">
                ...
                <div class="bg-white rounded-2xl overflow-hidden shadow-2xl transform transition-all w-full max-w-sm relative"
                    ...
                    <div class="bg-white px-6 pt-8 pb-6 text-center">
                        <div class="mx-auto flex items-center justify-center h-20 w-20 rounded-full bg-orange-50 mb-5">
                            <svg class="h-10 w-10 text-orange-500" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                            </svg>
        ...
        <div id="summary_modal" class="hidden fixed inset-0 z-[10000] overflow-y-auto" role="dialog" aria-modal="true">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
                ...
                <div
                    ...
                    <div
                        class="bg-gradient-to-r from-orange-500 to-orange-600 px-6 py-4 flex justify-between items-center">
