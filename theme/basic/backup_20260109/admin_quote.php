...
// [NEW] AJAX: Create Quote (First Sync)
if ($w == 'ajax_create_quote') {
    $qa_subject = trim($_POST['qa_subject'] ?? '신규 견적');
    ...
    $sql = " INSERT INTO g5_quote SET 
                ...
                qa_client_email = '" . trim($_POST['qa_client_email'] ?? '') . "',
                mb_id = '{$member['mb_id']}',
...
// [NEW] AJAX: Save Header Info (Step 1-2-3 Real-time Sync)
if ($w == 'ajax_save_header') {
    ...
    $upd = [
        'qa_subject' => trim($_POST['qa_subject'] ?? ''),
...
if ($w == 'ajax_list') {
    ...
    if ($q) {
        ...
        $q_safe = addslashes($q);
        $sql_search .= " and (qa_client_name like '%$q_safe%' or qa_code like '%$q_safe%' or qa_tax_company_name like '%$q_safe%') ";
    }
    ...
    if ($f_status) {
        ...
        $sql_search .= " and qa_status = '$f_status_safe' ";
    }
        ...
        <tr class="hover:bg-orange-50 transition border-b border-gray-100"
            ...
            <td class="p-3 text-center">
                <input type="checkbox" name="chk_qa_id[]" value="<?php echo $row['qa_id']; ?>"
                    id="chk_qa_id_<?php echo $row['qa_id']; ?>"
                    class="chk_box w-4 h-4 rounded border-gray-300 text-orange-600 focus:ring-orange-500">
            </td>
            <td class="p-3">
            ...
            <td class="p-3 text-center">
                <a href="?q=<?php echo urlencode($row['qa_tax_company_name']); ?>" onclick="event.stopPropagation();"
...
// Save Quote
if ($w == 'u' || $w == 'c') {
    check_quote_token(); // CSRF Check

    $qa_subject = addslashes($_POST['qa_subject']);
    $qa_client_name = addslashes($_POST['qa_client_name']);
    $qa_client_email = addslashes($_POST['qa_client_email']);
    $qa_deposit = (int) $_POST['qa_deposit'];
    $qa_client_hp = addslashes($_POST['qa_client_hp']);
    $qa_client_contact = addslashes($_POST['qa_client_contact']);
    $qa_client_addr = addslashes($_POST['qa_client_addr']);
    $qa_client_addr2 = addslashes($_POST['qa_client_addr2']);
    $qa_memo = addslashes($_POST['qa_memo']); // Internal Memo
    $qa_memo_user = addslashes($_POST['qa_memo_user']); // Customer Memo
    // [NEW] Company Name
    $qa_tax_company_name = addslashes($_POST['qa_tax_company_name']);
    $qa_code = addslashes($_POST['qa_code']);
    $qa_status = addslashes($_POST['qa_status'] ?? '작성중'); // Status

    // Items
    $qi_item = $_POST['qi_item'] ?? []; // Fix Warning if empty
...
<!-- DEBUG: $w = '<?php echo htmlspecialchars($w); ?>' -->

<?php if ($w == ''): // -------------------- LIST VIEW -------------------- ?>
    <!-- DEBUG: 목록 영역 시작 -->
    <?php $hide_header_inputs = true; // 목록 페이지에서는 헤더 입력 필드 숨김 ?>

    <!-- Sidebar Layout Wrapper for List View -->
    <div class="w-full min-h-screen bg-gray-50/50 flex items-stretch">
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="container mx-auto px-4 py-6 max-w-7xl">
                <!-- 통합 헤더 및 스텝 네비게이션 포함 -->
                <?php include_once(G5_THEME_PATH . '/admin_quote_header.php'); ?>

                <div class="mt-4">
                    <!-- Search & Tabs -->
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 mb-6">
                        <!-- Top Control Bar -->
                        <form name="fsearch" id="fsearch" method="get" class="p-4 border-b border-gray-100"
                            ...
                            <div class="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center justify-between">
                                ...
                                <div class="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center flex-1">
                                    ...
                                    <div class="relative w-full sm:w-40">
                                        <select name="f_status" id="f_status" onchange="load_list()"
                                            class="w-full pl-3 pr-8 py-2.5 border border-gray-300 rounded-lg text-sm font-bold text-gray-700 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 bg-white">
                                    ...
                                    <div class="relative flex-1 max-w-md">
                                        <input type="text" name="q" id="q" value="<?php echo $q; ?>"
                                            ...
                                            class="border border-gray-300 rounded-lg pl-9 pr-4 py-2.5 text-sm w-full focus:ring-2 focus:ring-orange-500 focus:border-orange-500">
                                ...
                                <div class="flex gap-2 items-center">
                                    <button type="button" onclick="location.href='./admin_customer.php'"
                                        class="px-4 py-2.5 bg-white border-2 border-orange-500 text-orange-600 rounded-lg font-bold text-sm hover:bg-orange-50 transition whitespace-nowrap">
                                        + 고객등록
                                    </button>
                                    <button type="button" onclick="location.href='./admin_quote.php?w=form'"
                                        class="px-4 py-2.5 bg-orange-500 text-white rounded-lg font-bold text-sm hover:bg-orange-600 transition shadow-sm whitespace-nowrap">
                                        견적 작성
                                    </button>
                                </div>
                            </div>
...
<?php if ($w == 'form'): // -------------------- FORM VIEW -------------------- ?>
        ...
        // 실측 데이터 (Step 1) 조회 - 2단계 연동용 가이드 데이터
        $step1_measures = [];
        if ($qa_id) {
        ...
        ?>
        <script>
                var step1_mea sures = <?php echo json_encode($step1_measures); ?>;
            </script>
            <?php
                                        ...
                                        <div class="bg-white rounded-xl shadow-soft border border-gray-100 p-8">
                                            <div class="flex justify-between items-center mb-6">
                                                <h2 class="text-xl font-bold text-gray-800 flex items-center gap-2">
                                                    <span class="w-1.5 h-5 bg-orange-500 rounded-full"></span> 견적 내역 (Items)
                                                </h2>
                                                <span
                                                    class="text-xs text-gray-400 font-medium bg-gray-50 px-3 py-1.5 rounded-full">품목
                                                    입력 후
                                                    Enter를 누르면 행이 추가됩니다.</span>
                                            </div>
                                            ...
                                            <div class="overflow-x-auto -mx-8 px-8">
                                                <table class="w-full text-left border-collapse min-w-[800px]" id="tbl_items_new">
                                                    <thead
                                                        ...
                                                        <tr>
                                                            <th class="py-4 px-2 w-8"></th>
                                                            <th class="py-4 px-2 min-w-[120px]">품목 (ITEM)</th>
                                                            <th class="py-4 px-2 w-24 text-center">가로 (W)</th>
                                                            <th class="py-4 px-2 w-24 text-center">세로 (H)</th>
                                                            <th class="py-4 px-2 w-20 text-center">수량</th>
                                                            <th class="py-4 px-2 w-32 text-right">단가</th>
                                                            <th class="py-4 px-2 w-32 text-right">금액</th>
                                                            <th class="py-4 px-2 w-32">비고</th>
                                                            <th class="py-4 px-2 w-8 text-center"></th>
                                                        </tr>
                                                    </thead>
                                                    <tbody id="item_list_container" class="divide-y divide-gray-50">
                                                        <!-- Rows injected by JS -->
                                                    </tbody>
                                                </table>
                                            </div>
                                            ...
                                            <div
                                                class="mt-8 pt-8 border-t border-dashed border-gray-100 flex justify-center gap-4 relative z-10">
                                                <button type="button" id="btn_add_item_row"
                                                    class="flex items-center gap-2 px-8 py-3.5 bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 rounded-xl text-sm font-bold transition shadow-sm hover:shadow-md cursor-pointer">
                                                    <span class="text-lg">+</span> 품목 행 추가
                                                </button>
                                                <?php if ($qa_id): ?>
                                                            <button type="button" onclick="load_step1_data()"
                                                                class="flex items-center gap-2 px-8 py-3.5 bg-orange-500 text-white hover:bg-orange-600 rounded-xl text-sm font-bold transition shadow-soft cursor-pointer">
                                                                <i class="fas fa-file-import"></i> 1단계 실측 수치 불러오기
                                                            </button>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                                        ...
                                        <div class="bg-white rounded-xl shadow-soft border border-gray-100 p-8">
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                                <div class="input-group">
                                                    <h3 class="input-label mb-2 flex items-center gap-2">
                                                        🔒 내부 메모 <span
                                                            class="text-[11px] font-normal text-gray-400 bg-gray-50 px-2 py-0.5 rounded">고객
                                                            노출
                                                            X</span>
                                                    </h3>
                                                    <textarea name="qa_memo"
                                                        class="w-full h-40 p-4 border border-transparent bg-gray-50 rounded-xl resize-none text-sm placeholder-gray-400 focus:bg-white focus:border-orange-500 focus:ring-4 focus:ring-orange-500/5 transition outline-none"
                                                        placeholder="관리자 전용 메모입니다."><?php echo $quote['qa_memo']; ?></textarea>
                                                </div>
                                                <div class="input-group">
                                                    <h3 class="input-label mb-2 flex items-center gap-2">
                                                        📢 고객 참고사항 <span
                                                            class="text-[11px] font-normal text-orange-500 bg-orange-50 px-2 py-0.5 rounded">견적서
                                                            하단 표시</span>
                                                    </h3>
        ...
        <script>
            ...
            // ---------------------------------------------------------
            // 3. Form Submission (Missing Function Recovered)
            // ---------------------------------------------------------
            function fquote_submit(f) {
                // [FIX] 필수 항목 검사
                if (!f.qa_subject.value.trim()) {
                    alert('견적명(프로젝트명)을 입력해주세요.');
                    f.qa_subject.focus();
                    return false;
                }

                if (!f.qa_client_name.value.trim()) {
                    alert('고객명을 입력해주세요.');
                    f.qa_client_name.focus();
                    return false;
                }

                // 아이템 체크
                const items = document.getElementsByName('qi_item[]');
                let hasItem = false;
            ...
            // ---------------------------------------------------------
            // Load Step 1 Data (Selective Linking)
            // ---------------------------------------------------------
            function load_step1_data() {
                if (typeof step1_measures === 'undefined' || !step1_measures || step1_measures.length === 0) {
                    alert('불러올 실측 데이터가 없습니다. 1단계를 먼저 작성해 주세요.');
                    return;
                }

                if (!confirm('1단계의 실측 수치들을 품목 리스트에 추가하시겠습니까?\n(기존 입력된 내역 아래에 추가됩니다)')) {
                    return;
                }

                step1_measures.forEach(m => {
                    var spec = '';
                    if (m.qm_width && m.qm_height) spec = m.qm_width + '×' + m.qm_height;
                    else if (m.qm_width) spec = m.qm_width;
                    else if (m.qm_height) spec = m.qm_height;
                ...
                alert('실측 데이터가 추가되었습니다. 단가를 입력해 주세요.');
            }
        </script>
