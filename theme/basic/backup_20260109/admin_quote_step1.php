...
<div class="w-full min-h-screen bg-gray-50/50 flex items-stretch">
        ...
        <form id="step1Form" method="post" enctype="multipart/form-data" onsubmit="return beforeSubmit()"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-10 gap-8 items-start">
                ...
                <div class="lg:col-span-7 space-y-6">
                    ...
                    <div class="bg-white rounded-xl shadow-soft border border-gray-100 p-8">
                        <div class="flex items-center justify-between mb-8">
                            <h2 class="text-xl font-bold text-gray-800 flex items-center gap-2">
                                <span class="w-1.5 h-5 bg-orange-500 rounded-full"></span> 현장 실측 데이터
                            </h2>
                            <button type="button" onclick="addMeasureRow()"
                                class="px-5 py-2.5 bg-gray-900 text-white rounded-xl hover:bg-black flex items-center gap-2 text-sm font-bold transition shadow-sm">
                                <i class="fas fa-plus"></i> 항목 추가
                            </button>
                        </div>

                        <div id="measure_items" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <textarea name="qa_memo"
                                    ...
                                    placeholder="현장 상황, 요청사항 등 내부용 메모를 입력하세요."><?php echo $quote['qa_memo'] ?? ''; ?></textarea>
                            </div>
                            <div>
                                <h3 class="text-sm font-bold text-gray-700 mb-2 flex items-center gap-2">
                                    📢 고객 참고사항 <span
                                ...
                                <textarea name="qa_memo_user"
                                    class="w-full h-40 p-4 border border-transparent bg-gray-50 rounded-xl resize-none text-sm placeholder-gray-400 focus:bg-white focus:border-orange-500 focus:ring-4 focus:ring-orange-500/5 transition outline-none"
                                    placeholder="고객에게 전달할 현장 특이사항을 입력하세요."><?php echo $quote['qa_memo_user'] ?? ''; ?></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    </div> <!-- [FIX] Close Action Buttons Flex -->
                </div> <!-- [FIX] Close lg:col-span-7 Area -->
                <?php include_once(G5_THEME_PATH . '/admin_quote_sidebar.php'); ?>
            </div> <!-- [FIX] Close lg:grid-cols-10 Container -->
        </form>
    </div> <!-- [FIX] Close mt-4 Container -->
    ...
    `;
        ...
        // Delete Image (for existing/saved images)
        function deleteImage(btn) {
            open_confirm('이미지를 삭제하시겠습니까?', function () {
                const row = btn.closest('.measure-row');
                // If it's a preview (not saved), we can just remove it
                // But if it's a saved image, we need to mark for deletion

                // Check if this is a preview only
                const imgContainer = btn.closest('div').closest('div'); // Parent div of preview container

                // Mark hidden del input
                ...
                // Remove preview
                btn.parentElement.remove(); // Remove the relative div containing img and btn
                hasUnsavedChanges = true;
            });
        }
        ...
        function handleImagePreview(input) {
            if (input.files && input.files[0]) {
                ...
                reader.onload = function (e) {
                    ...
                    // Structure: Label -> Span+Input
                    // Preview Container is sibling of Label
                    const parentDiv = input.closest('div');
                    let previewContainer = parentDiv.querySelector('.preview-container');

                    if (!previewContainer) {
                        // Create if missing (should not happen with new structure)
                    ...
                    btn.innerHTML = '×';
                    btn.onclick = function () { removePreview(this); }; // Distinct function for fresh previews
...
<div id="custom_confirm_modal" class="hidden fixed inset-0 z-[9999]" aria-labelledby="modal-title" role="dialog"
    ...
    </div> <!-- Close modal centering div -->
</div> <!-- Close confirm_modal dialog -->
            </div>
        </div>
    </div>
</div>
