...
<div class="w-full min-h-48 flex flex-col justify-center items-center py-12 bg-black/30 bg-center bg-cover bg-no-repeat section-bg-image bg-[url('<?php echo G5_THEME_IMG_URL ?>/board_guide_header.jpg')]">
...
<div id="bo_gall" style="width:<?php echo $width; ?>" class="pb-12 container">    
    ...
    <?php 
        ...
        <?php } else { ?>
            <img class="h-auto lg:w-[1248px] object-cover rounded-2xl shadow-md" src="<?php echo $image_src; ?>" alt="첫 번째 게시글 이미지" />
        <?php } ?>
        </div>
    ...
    <div class="portfolio-list__grid w-full grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-12">
        ...
        <?php
            ...
            $img_src = $thumb['src'] ? $thumb['src'] : 'https://placehold.co/400x192';
            ...
            <div class="portfolio-list__info w-full h-24 p-4 flex flex-col justify-start items-start">
                ...
                <div class="w-full h-10 flex justify-start items-start flex-wrap content-start overflow-hidden">
                    <div class="text-gray-700 text-sm leading-tight w-full line-clamp-2">
                        ...
                        // 이미지 관련 텍스트 제거 (img 태그가 있었던 경우)
                        $content = preg_replace('/\[이미지\d*\]/', '', $content);
                        // 연속된 공백 제거
