...
<div class="w-full relative overflow-hidden bg-[#0a192f] text-white">
    <!-- Background Pattern/Effect -->
    <div class="absolute inset-0 bg-gradient-to-r from-[#0a192f] to-[#112d4e] opacity-90 z-0"></div>
    <div class="absolute inset-0 opacity-10"
        style="background-image: url('<?php echo G5_THEME_IMG_URL ?>/pattern_dot.png');"></div>

    <div
        class="container mx-auto px-4 py-16 relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
        <!-- Left: Title Section -->
        <div class="text-center md:text-left space-y-2">
            <div
                class="inline-block px-3 py-1 bg-amber-500/20 text-amber-400 rounded-full text-sm font-bold mb-2 border border-amber-500/30">
                ✨ 간판제작 고객 만족도 TOP!
            </div>
            <h1 class="text-4xl md:text-5xl font-extrabold tracking-tight">
                <span class="text-white">간판의품격</span>
                <span class="text-amber-400 relative inline-block">
                    고객 이용 후기
                    <svg class="absolute -bottom-2 delay-300 left-0 w-full h-2 text-amber-500 opacity-50"
        ...
        <!-- Right: Ratings Badge -->
        <div
            class="flex items-center gap-6 bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10 shadow-2xl transform hover:scale-105 transition-transform duration-300">
            <div class="text-right">
                <div class="text-gray-300 text-sm font-medium mb-1">숨고 / 당근마켓 리뷰 평점</div>
                <div class="text-5xl font-bold text-white flex items-center justify-end gap-2">
                    5.0
                    <span class="text-2xl text-amber-400">점</span>
                </div>
                <div class="flex text-amber-400 text-xl mt-1 justify-end">
                    ⭐⭐⭐⭐⭐
                </div>
            </div>
            <div class="h-16 w-px bg-white/20"></div>
            <div class="text-center">
                <div class="text-gray-300 text-xs mb-1">총 누적 후기</div>
                <div class="text-2xl font-bold text-white"><?php echo number_format($total_count) ?>+</div>
                <div class="text-xs text-amber-400 mt-1">실시간 집계중</div>
            </div>
        </div>
    </div>
</div>
                ...
                foreach ($list as $i => $row) { ?>
                    ...
                    <div
                        ...
                        <a href="<?php echo $row['href']; ?>" class="w-full h-full">
                            ...
                            <div class="portfolio-list__info w-full flex-1 p-5 flex flex-col">
                                <!-- [NEW] 별점 & 지역 표시 -->
                                <div class="flex justify-between items-center mb-2">
                                    <div class="flex text-amber-400 text-sm">
                                        <?php
                                        $star_score = (int) ($row['wr_1'] ? $row['wr_1'] : 5); // 기본 5점
                                        for ($k = 1; $k <= 5; $k++) {
                                            if ($k <= $star_score)
                                                echo '★';
                                            else
                                        ...
                                        ?>
                                        <span class="ml-1 text-gray-400 text-xs">(<?php echo $star_score ?>.0)</span>
                                    </div>
                                    <?php if ($row['wr_2']) { ?>
                                        <span
                                            class="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-full border border-gray-200">
                                            📍 <?php echo $row['wr_2'] ?>
                                        </span>
                                    <?php } ?>
                                </div>
                                ...
                                <div class="pb-2 flex justify-start items-start w-full">
                                    <div
                                        class="text-slate-800 text-lg font-bold leading-7 w-full truncate portfolio-list__title group-hover:text-orange-600 transition-colors">
