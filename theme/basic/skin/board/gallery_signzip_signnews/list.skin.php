...
<div id="bo_gall" style="width:<?php echo $width; ?>" class="pb-12 container">
    ...
    <?php if ($is_category) { ?>
        <nav id="bo_cate" class="w-full flex justify-center items-center">
        ...
        <div class="portfolio-list__grid w-full grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-12">
            <?php if (count($list)) {
                foreach ($list as $i => $row) { ?>
                    ...
                    ?>
                    <div
                        class="portfolio-list__card bg-white rounded-2xl shadow-lg drop-shadow-md flex flex-col justify-start items-start overflow-hidden hover:shadow-xl transition-shadow relative">
                        <?php if ($is_admin) { ?>
                        ...
                        <a href="<?php echo $row['href']; ?>" class="w-full h-full">
                            ...
                            <div class="portfolio-list__info w-full h-24 p-4 flex flex-col justify-start items-start">
                                ...
                                <div class="pb-1 flex justify-start items-start w-full">
                                    <div class="text-gray-500 text-xs leading-tight w-full truncate">
                                        <?php if ($row['is_notice']) { ?>
                                            <span
                                                class="inline-flex items-center px-2 bg-accent text-black text-xs font-bold rounded-full">공지</span>
                                        <?php } else { ?>
                                            <span class="text-gray-600">NO. <?php echo $row['wr_num']; ?> |
                                                <?php echo $row['ca_name']; ?></span>
                                        <?php } ?>
                                    </div>
                                </div>
                                ...
                                <div class="flex justify-start items-start w-full">
                                    <div class="text-gray-400 text-xs leading-tight w-full truncate">
                                        <span class="text-gray-600"><?php echo date('Y-m-d', strtotime($row['wr_datetime'])); ?>
                                    </div>
                                </div>
                            </div>
                        </a>
        ...
        <?php if ($write_href) { ?>
            <div class="flex justify-end">
                <a href="<?php echo $write_href; ?>"
                    class="w-24 h-10 px-4 py-2 bg-accent rounded-lg inline-flex justify-start items-center cursor-pointer hover:bg-lime-500 transition-colors">
    ...
    <!-- 퀵상담 CTA 배너 -->
    <section x-data="{ modalOpen: false }" class="container pt-12">
        <div
            class="w-full max-w-4xl mx-auto p-4 sm:p-6 bg-accent rounded-2xl flex flex-col md:flex-row justify-between items-center gap-4 sm:gap-6">
            ...
            <div>
                <a href="<?php echo G5_BBS_URL . '/write.php?bo_table=consult'; ?>"
                    class="w-full md:w-auto px-6 py-3 bg-white rounded-full shadow flex justify-center items-center font-bold text-gray-900 text-base text-center transition hover:bg-gray-100">
                    퀵 상담 신청하기
                </a>
            </div>
    </section>
