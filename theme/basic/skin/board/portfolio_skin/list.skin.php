        ...
        <main class="flex-1 min-w-0">
            ...
            <div class="flex justify-between items-end mb-8">
                <div>
                    ...
                    <p class="text-gray-500 mt-2">
                        총 <strong class="text-brand-orange"><?php echo number_format($total_count) ?></strong>개의 상품이
                        있습니다.
                    </p>
                </div>
                <?php if ($write_href) { ?>
            ...
            <?php if (!$sca) { // 전체 목록일 때만 배너 노출 ?>
                <div class="relative w-full h-64 rounded-3xl overflow-hidden mb-12 bg-gray-800 flex items-center">
                    ...
                    <img src="https://images.unsplash.com/photo-1542206395-9feb3edaa68d?q=80&w=2000&auto=format&fit=crop"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-x-6 gap-y-10">
                ...
                for ($i = 0; $i < count($list); $i++) {
                    ...
                    <a href="<?php echo $row['href'] ?>" class="group block product-card">
                        <div
                            class="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300">
                            <div class="relative aspect-square overflow-hidden bg-gray-50">
                                ...
                                <div class="absolute top-3 left-3 flex flex-col gap-1">
                                    <?php foreach ($badges as $badge) { ?>
                                        <span
                                            class="px-2.5 py-1 <?php echo $badge['bg'] ?> text-white text-[10px] font-bold rounded shadow-sm">
                                            <?php echo $badge['text'] ?>
                                        </span>
                                    <?php } ?>
                                </div>
                                ...
                                <div
                                    class="absolute bottom-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity translate-y-2 group-hover:translate-y-0 duration-300">
                                    <div
                                        class="w-10 h-10 bg-brand-orange rounded-full flex items-center justify-center shadow-lg text-white hover:bg-orange-700 transition-colors">
                            ...
                            <div class="p-5">
                                <div class="text-xs text-gray-400 mb-1">
                                    <?php echo $row['ca_name'] ? $row['ca_name'] : 'Item' ?>
                                </div>
                                <h3
                                    class="text-lg font-bold text-gray-900 truncate mb-2 group-hover:text-brand-orange transition-colors">
                                    <?php echo $row['subject'] ?>
                                </h3>
                                <div class="text-sm text-gray-500 font-medium line-clamp-1 mb-2">
                                ...
                                <div class="mt-auto text-left">
                                    <div
                                        ...
                                        <span class="text-base font-bold text-gray-700 leading-none">
                                            <?php
                                            if ($is_member) {
                                                echo htmlspecialchars($price_value); ?>만원대
                                            <?php } else { ?>
                                                <span class="neon-text text-lg">???</span> <span
                                                    class="text-xs text-gray-400">만원대</span>
                                            <?php } ?>
                                        </span>
                                    </div>
                                </div>
