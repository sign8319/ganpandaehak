...
<script>
// 글자수 제한 변수 초기화
var char_min = parseInt(<?php echo $comment_min ?: 0 ?>); // 최소
var char_max = parseInt(<?php echo $comment_max ?: 0 ?>); // 최대
...
function fviewcomment_submit(f)
{
    ...
    // 비회원인 경우 이름과 비밀번호 체크
    if (typeof(f.wr_name) != 'undefined' && !f.wr_name.value.trim()) {
        alert('이름이 입력되지 않았습니다.');
        f.wr_name.focus();
        return false;
    }

    if (typeof(f.wr_password) != 'undefined' && !f.wr_password.value.trim()) {
    ...
    // 토큰 설정 (함수가 있는 경우에만)
    if (typeof set_comment_token === 'function') {
        set_comment_token(f);
    }
