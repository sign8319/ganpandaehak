...
<style>
    ...
    /* 본문 이미지 자동 리사이징 */
    #bo_v_con img { max-width: 100% !important; height: auto !important; border-radius: 8px; }
</style>
...
<div class="w-full max-w-[1600px] mx-auto px-4 py-12">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
        ...
        <div class="lg:sticky lg:top-24 h-fit">
            ...
            ?>
            <div class="p-6 bg-gray-50 rounded-xl mb-8 border border-gray-100">
                <div class="text-sm text-gray-500 mb-1">판매가</div>
                <div class="flex items-baseline gap-1">
                    <span class="text-4xl font-black text-brand-orange"><?php echo $price; ?></span>
                    <span class="text-xl text-gray-600 font-bold"><?php echo $price_unit; ?></span>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-3">
                <div class="flex gap-3">
                    <button type="button" onclick="alert('결제 서비스 준비 중입니다. 견적 문의를 이용해 주세요.');" class="flex-1 py-4 bg-gray-800 text-white rounded-xl font-bold text-lg hover:bg-black transition-colors shadow-lg">
                        구매하기
                    </button>
                    <a href="<?php echo G5_BBS_URL ?>/write.php?bo_table=consult&sca=견적문의" class="flex-1 py-4 bg-brand-orange text-white rounded-xl font-bold text-lg hover:bg-orange-600 transition-colors shadow-lg text-center flex items-center justify-center">
                        견적 문의
                    </a>
                </div>
                ...
                <div class="flex justify-end gap-3 mt-4 pt-4 border-t border-gray-200">
                    <?php if ($update_href) { ?>
                    <a href="<?php echo $update_href ?>" class="px-3 py-1 bg-white border border-gray-300 rounded text-sm text-gray-600 hover:bg-gray-50">수정</a>
                    <?php } ?>
                    <?php if ($delete_href) { ?>
                    <a href="<?php echo $delete_href ?>" onclick="del(this.href); return false;" class="px-3 py-1 bg-white border border-red-200 text-red-500 rounded text-sm hover:bg-red-50">삭제</a>
                    <?php } ?>
                    <a href="<?php echo $list_href ?>" class="px-3 py-1 bg-gray-100 rounded text-sm text-gray-600 hover:bg-gray-200">목록으로</a>
                </div>
                <?php } ?>
            </div>
        </div>
