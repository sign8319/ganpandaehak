...
<!-- SKIN UPDATED: <?php echo date('Y-m-d H:i:s'); ?> (Aspect Ratio Fixed 1:1) -->
<!-- MEMBER STATUS: <?php echo $is_member_check ? 'MEMBER' : 'GUEST'; ?> -->
...
<div class="relative w-full max-w-7xl mx-auto px-4 py-8">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6">
        ...
        for ($i = 0; $i < count($list); $i++) {
            $img_content = '<img src="https://placehold.co/400x400/f3f4f6/9ca3af?text=No+Image" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" alt="' . $list[$i]['subject'] . '">';
                ...
                <a href="<?php echo $list[$i]['href'] ?>"
                    ...
                    <div class="p-3 md:p-6 flex flex-col flex-1 text-left">
                        ...
                        <?php if ($location_clean) { ?>
                            <div class="flex items-center gap-1 text-[10px] md:text-xs text-gray-500 mb-1 md:mb-2">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                                ...
                                <span
                                    class="truncate max-w-[80px] md:max-w-none"><?php echo htmlspecialchars($location_clean); ?></span>
                            </div>
                        <?php } ?>

                        <!-- 제목 -->
                        <h3
                            class="font-bold text-gray-900 text-sm md:text-lg mb-1 md:mb-2 line-clamp-1 md:line-clamp-2 h-auto md:h-14 group-hover:text-brand-orange transition-colors leading-tight">
                        ...
                        <div class="text-[10px] text-gray-400 mb-2 md:mb-4 cursor-pointer hidden md:block">더보기</div>
                                ...
                                if ($list[$i]['wr_2'] && mb_strlen($wr_2_clean, 'utf-8') < 20) {
                                    ...
                                    <span class="text-sm md:text-base font-black text-gray-900 ml-0 md:ml-1">
                                        <?php if ($is_member_check) { ?>
                                            <?php echo $list[$i]['wr_2']; ?><span
                                                class="text-[10px] md:text-xs text-gray-900 font-bold">만원대</span>
                                        <?php } else { ?>
                                            <span class="neon-text text-sm md:text-lg">???</span> <span
                                                class="text-[10px] md:text-xs text-gray-400 font-bold">만원대</span>
                                        <?php } ?>
                                    </span>
                                <?php } else { ?>
