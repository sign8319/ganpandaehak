...
<div id="mb_login_modal_wrap" class="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm">
    <div class="login_modal_box bg-white rounded-3xl p-10 w-full max-w-[480px] shadow-2xl relative overflow-hidden animate-fade-in-up">
        ...
        <!-- Legacy Form -->
        <form name="flogin" action="<?php echo $login_action_url ?>" onsubmit="return flogin_submit(this);" method="post" class="flex flex-col gap-4">
            <input type="hidden" name="url" value="<?php echo $login_url ?>">

            <div>
                <label for="login_id" class="sr-only">아이디</label>
                <input type="text" name="mb_id" id="login_id" required class="w-full h-12 px-4 bg-gray-50 rounded-xl border-none focus:ring-2 focus:ring-orange-500 text-sm transition-shadow" placeholder="아이디">
            </div>
            
            <div>
                <label for="login_pw" class="sr-only">비밀번호</label>
                <input type="password" name="mb_password" id="login_pw" required class="w-full h-12 px-4 bg-gray-50 rounded-xl border-none focus:ring-2 focus:ring-orange-500 text-sm transition-shadow" placeholder="비밀번호">
            </div>

            <div class="flex items-center justify-between text-xs text-gray-400 mt-1">
                <div class="flex items-center gap-1">
                    <input type="checkbox" name="auto_login" id="login_auto_login" class="rounded border-gray-300 text-orange-500 focus:ring-orange-500 w-4 h-4">
                    <label for="login_auto_login">자동로그인</label>
                </div>
                <!-- ID/PW Finding Link (Optional) -->
                 <a href="<?php echo G5_BBS_URL ?>/password_lost.php" class="hover:text-gray-600 underline-offset-2 hover:underline">ID/PW 찾기</a>
            </div>

            <button type="submit" class="w-full h-12 mt-2 rounded-xl bg-gray-900 text-white font-bold text-sm hover:bg-black transition-colors">
                로그인
            </button>
            <div class="mt-4 text-center">
                <a href="<?php echo G5_BBS_URL ?>/register.php" class="text-sm text-gray-500 underline hover:text-gray-800">
                    아직 회원이 아니신가요?
                </a>
            </div>
        </form>
